...
var socket = io('http://203.24.50.82:1111'); 
...
function req_jam(){
    $.getJSON( "http://203.24.50.82:2020/laporan", function( data ) {
 ...
 $('.date').pickadate({
    ...
    $('#cek_data').on('click', function(){ 
        ...
        if(tanggal==today){
            ...
            $.getJSON('http://203.24.50.82:2020/backuplogs/'+tanggal+'',function(data){
            console.log(data.visit_time);
            var newArr = data.visit_time.data.filter(function(el){
                return el.data ===  arrwaktu[0];
                });
        ...
        $.getJSON( 'http://203.24.50.82:2020/logs/'+tanggal+'/'+arrwaktu[0]+'', function( data ) {
            // console.log(data);   
			var arrmenit_cpu = [];
            var arrfixmenit= [];
